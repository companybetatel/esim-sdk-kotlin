package com.betatel.esim.infrastructure
...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    companion object {
        ...
        const val baseUrlKey: String = "com.betatel.esim.baseUrl"
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        ...
        if (requestConfig.headers["x-user-id"].isNullOrEmpty()) {
            if (apiKey["x-user-id"] != null) {
                if (apiKeyPrefix["x-user-id"] != null) {
                    requestConfig.headers["x-user-id"] = apiKeyPrefix["x-user-id"]!! + " " + apiKey["x-user-id"]!!
                } else {
                    requestConfig.headers["x-user-id"] = apiKey["x-user-id"]!!
                }
            }
        }
    }
